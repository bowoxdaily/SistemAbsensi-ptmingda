...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="fw-bold mb-0">
                <span class="text-muted fw-light">Rekrutmen /</span> join_call
            </h4>
            <div>
                <button type="button" class="btn btn-success me-2" id="importBtn">
                    <i class='bx bx-import me-1'></i> Import Excel
                </button>
                <button type="button" class="btn btn-primary" id="addjoin_callBtn">
                    <i class='bx bx-plus me-1'></i> Jadwalkan Panggilan Join
                </button>
            </div>
        </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <small class="text-muted d-block">Total Panggilan Join</small>
                ...
                <form method="GET" id="filterForm">
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <a href="{{ route('admin.join_calls.index') }}" class="btn btn-secondary">
        ...
        <div class="card mt-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">Daftar Kandidat Panggilan Join</h5>
                <div class="d-flex align-items-center">
            ...
            <div class="card-body">
                ...
                <div class="table-responsive d-none d-lg-block">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @forelse($join_calls as $index => $join_call)
                                <tr>
                                    <td>
                                        <input type="checkbox" class="form-check-input join_call-checkbox" 
                                               value="{{ $join_call->id }}"
                                               data-name="{{ $join_call->candidate_name }}">
                                    </td>
                                    <td>{{ $join_calls->firstItem() + $index }}</td>
                                    ...
                                    <td>
                                        @if ($join_call->status == 'scheduled')
                                            <span class="badge bg-warning">Terjadwal</span>
                                        @elseif($join_call->status == 'notified')
                                            <span class="badge bg-info">Ternotifikasi</span>
                                        @elseif($join_call->status == 'confirmed')
                                            <span class="badge bg-success">Dikonfirmasi</span>
                                        @elseif($join_call->status == 'completed')
                                    ...
                                    <td>
                                        @if($join_call->wa_sent_at)
                                            <i class='bx bx-check-circle text-success'></i>
                                            <small>{{ \Carbon\Carbon::parse($join_call->wa_sent_at)->diffForHumans() }}</small>
                                        @else
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item detail-btn" href="javascript:void(0);" data-id="{{ $join_call->id }}">
                                                    <i class="bx bx-detail me-1"></i> Detail
                                                </a>
                                                <a class="dropdown-item edit-btn" href="javascript:void(0);" data-id="{{ $join_call->id }}">
                                                    <i class="bx bx-edit me-1"></i> Edit
                                                </a>
                                                <a class="dropdown-item send-wa-btn" href="javascript:void(0);" data-id="{{ $join_call->id }}">
                                                    <i class="bx bx-send me-1"></i> Kirim Notifikasi
                                                </a>
                                                <a class="dropdown-item view-qr-btn" href="javascript:void(0);" data-id="{{ $join_call->id }}" data-name="{{ $join_call->candidate_name }}" data-token="{{ $join_call->qr_code_token }}">
                                                    <i class="bx bx-qr me-1"></i> Lihat QR Code
                                                </a>
                                                <a class="dropdown-item text-danger delete-btn" href="javascript:void(0);" 
                                                   data-id="{{ $join_call->id }}" data-name="{{ $join_call->candidate_name }}">
                            ...
                            @empty
                                <tr>
                                    <td colspan="11" class="text-center py-4">
                                        ...
                                        <p class="mt-2 mb-0 text-muted">Belum ada jadwal join_call</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <!-- Mobile Card View -->
                <div class="d-lg-none">
                    @forelse($join_calls as $join_call)
                        <div class="card mb-3 shadow-sm">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start mb-3">
                                    <div class="d-flex align-items-start gap-2 flex-grow-1">
                                        <input type="checkbox" class="form-check-input join_call-checkbox mt-1" 
                                               value="{{ $join_call->id }}" data-name="{{ $join_call->candidate_name }}">
                                        <div>
                                ...
                                <div class="mb-2">
                                    <small class="text-muted d-block mb-1">
                                        <i class='bx bx-envelope'></i> {{ $join_call->email ?? '-' }}
                                    </small>
                                    <small class="text-muted d-block mb-1">
                                        <i class='bx bxs-phone'></i> {{ $join_call->phone ?? '-' }}
                                    </small>
                                    <small class="text-muted d-block mb-1">
                                        <i class='bx bx-calendar'></i> {{ \Carbon\Carbon::parse($join_call->join_call_date)->locale('id')->translatedFormat('d M Y') }}
                                    </small>
                                    <small class="text-muted d-block">
                                        <i class='bx bx-time'></i> {{ \Carbon\Carbon::parse($join_call->join_call_time)->format('H:i') }} WIB
                                    </small>
                                </div>
                ...
                <div class="mt-3 d-flex justify-content-between align-items-center flex-wrap">
                    <div class="mb-2 mb-md-0">
                        <span class="text-muted">
                            Menampilkan {{ $join_calls->firstItem() ?? 0 }} - {{ $join_calls->lastItem() ?? 0 }}
                            dari {{ $join_calls->total() }} data
                        </span>
                    </div>
                    <div>
                        {{ $join_calls->onEachSide(1)->links('pagination::bootstrap-5') }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- Add/Edit Modal -->
    <div class="modal fade" id="join_callModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTitle">Jadwalkan Panggilan Join</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form id="join_callForm">
                    <div class="modal-body">
                        <input type="hidden" id="join_call_id" name="id">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" class="form-control" id="join_call_date" name="join_call_date" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Waktu Join <span class="text-danger">*</span></label>
                                <input type="time" class="form-control" id="join_call_time" name="join_call_time" required>
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Lokasi Join</label>
    ...
    <div class="modal fade" id="detailModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Detail Panggilan Join</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
    ...
    <div class="modal fade" id="importModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Import Data Panggilan Join</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info">
                            ...
                            <ol class="mb-0 mt-2 ps-3">
                                ...
                                <li><strong>Kolom WAJIB diisi:</strong> Nama Kandidat, Email, Departemen, Tanggal Join, Waktu Join</li>
                        ...
                        <div class="mb-3">
                            ...
                            <div>
                                <a href="/api/admin/join-calls/template/download" class="btn btn-sm btn-outline-primary">
    ...
    <script>
        let messageTemplates = [];
            ...
            function loadTemplates() {
                $.ajax({
                    url: '/api/admin/join-calls/templates/list',
                    method: 'GET',
            ...
            // Add join_call
            $('#addjoin_callBtn').on('click', function() {
                $('#modalTitle').text('Jadwalkan Panggilan Join');
                $('#join_callForm')[0].reset();
                $('#join_call_id').val('');
                $('#custom_message_template').val('');
                $('#statusGroup').hide();
                updateCharCount();
                const modal = new bootstrap.Modal(document.getElementById('join_callModal'));
                modal.show();
            });
            ...
            $('#importForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: '/api/admin/join-calls/import',
                    method: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
            ...
            // Save join_call (Add/Edit)
            $('#join_callForm').on('submit', function(e) {
                e.preventDefault();

                const id = $('#join_call_id').val();
                const url = id ? `/api/admin/join-calls/${id}` : '/api/admin/join-calls';
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                $.ajax({
                    url: `/api/admin/join-calls/${id}`,
                    method: 'GET',
                    success: function(response) {
                        const data = response.data;

                        $('#modalTitle').text('Edit join_call');
                        $('#join_call_id').val(data.id);
                        ...
                        if (match) {
                            $('#join_call_date').val(`${match[1]}-${match[2]}-${match[3]}`);
                        }

                        $('#join_call_time').val(data.join_call_time ? data.join_call_time.substring(0, 5) : '');
                    ...
                    error: function() {
                        Swal.fire('Error', 'Gagal memuat data join_call', 'error');
                    }
                });
            });

            // Detail Panggilan Join
            $(document).on('click', '.detail-btn', function() {
                ...
                $.ajax({
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <table class="table table-sm">
                                        ...
                                        <tr><th>Departemen</th><td>${data.sub_department?.name || data.subDepartment?.name || '-'}</td></tr>
                                    </table>
                                </div>
                                <div class="col-md-6">
                    ...
                    error: function() {
                        $('#detailContent').html('<div class="alert alert-danger">Gagal memuat Detail Panggilan Join</div>');
                    }
                });
            });
            ...
            $(document).on('click', '.delete-btn', function() {
                ...
                Swal.fire({
                    title: 'Hapus join_call?',
                    html: `Apakah Anda yakin ingin menghapus join_call untuk:<br><strong>${name}</strong>?`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            error: function() {
                                Swal.fire('Error', 'Gagal menghapus join_call', 'error');
                            }
                        });
                    }
                });
            ...
            $(document).on('click', '.send-wa-btn', function() {
                ...
                Swal.fire({
                    ...
                    text: 'Undangan join call akan dikirim ke email kandidat dan WhatsApp jika nomor tersedia',
                    icon: 'question',
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: `/api/admin/join-calls/${id}/send-notification`,
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Accept': 'application/json'
            ...
            $(document).on('click', '.view-qr-btn', function(e) {
                ...
                if (!token || token === '' || token === 'null') {
                    Swal.fire({
                        ...
                        text: 'QR Code akan di-generate otomatis saat join_call dibuat. Silakan refresh halaman.'
                    });
                    return;
                }
            ...
            function printQRCode() {
                ...
                printWindow.document.write(`
                    ...
                    <body>
                        <h2>QR Code Check-in join_call</h2>
                        <h3>${candidateName}</h3>
            ...
            $('#selectAll').on('change', function() {
                $('.join_call-checkbox:visible').prop('checked', $(this).prop('checked'));
                updateBulkButton();
            });

            $(document).on('change', '.join_call-checkbox', function() {
                updateBulkButton();
            });

            function updateBulkButton() {
                const count = $('.join_call-checkbox:visible:checked').length;
                if (count > 0) {
                    $('#bulkBlastBtn').show();
                    $('#bulkDeleteBtn').show();
            ...
            $('#bulkDeleteBtn').on('click', function() {
                ...
                $('.join_call-checkbox:visible:checked').each(function() {
                    ids.push($(this).val());
                ...
                Swal.fire({
                    title: 'Hapus join_call Terpilih?',
                    html: `Apakah Anda yakin ingin menghapus <strong>${ids.length}</strong> join_call berikut?${namesList}`,
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#d33',
                    cancelButtonColor: '#3085d6',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '/api/admin/join-calls/bulk-delete',
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Content-Type': 'application/json'
                            ...
                            success: function(response) {
                                Swal.fire({
                                    ...
                                    html: `<strong>${response.data.deleted}</strong> join_call berhasil dihapus`,
                            ...
                            error: function(xhr) {
                                Swal.fire(
                                    'Error',
                                    xhr.responseJSON?.message || 'Terjadi kesalahan saat menghapus join_call',
                                    'error'
                                );
                            }
                        });
            ...
            $('#bulkBlastBtn').on('click', function() {
                ...
                Swal.fire({
                    ...
                    html: `Undangan join call akan dikirim ke email kandidat, dan ke WhatsApp bila nomor tersedia:${namesList}`,
                    icon: 'question',
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '/api/admin/join-calls/bulk-send-notification',
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Accept': 'application/json',
            ...
            $('#saveAsTemplateBtn').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '/api/admin/join-calls/templates/save',
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Accept': 'application/json',
            ...
            $(document).on('click', '.delete-template-btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `/api/admin/join-calls/templates/${id}`,
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Accept': 'application/json'
            ...
            $('#previewMessageBtn').on('click', function() {
                ...
                if (join_callDate) {
                    const date = new Date(join_callDate);
                ...
                // Format time
                const formattedTime = join_callTime || '[Waktu]';

                // Use template if provided, otherwise use default
                let message = template || `*Undangan join_call - PT Mingda*
...
Terima kasih dan sampai jumpa di hari join_call.
